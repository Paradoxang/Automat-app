'use strict';
// Genera build/icon.ico y build/icon.png en Node puro (sin Electron).
// Degradado índigo→cian con esquinas redondeadas + avión de papel blanco.
// Uso: node tools/makeicon.js
const fs = require('fs');
const path = require('path');
const { PNG } = require('pngjs');
const pngToIco = require('png-to-ico').default;

const C0 = [0x63, 0x66, 0xf1]; // #6366f1 indigo
const C1 = [0x22, 0xd3, 0xee]; // #22d3ee cian
// Avión de papel "send" en espacio 24x24 (polígono de 6 vértices).
...
// Distancia con signo a un rectángulo redondeado SxS (<=0 dentro).
...
function renderSize(S) {
  const F = 4, B = S * F; // supersampling 4x
...
(async function main() {
  ...
  console.log('ICON_OK icon.ico ' + ico.length + ' bytes, icon.png ' + buffers[0].length + ' bytes');
})().catch((e) => { console.log('ICON_ERROR ' + e.message); process.exit(1); });
